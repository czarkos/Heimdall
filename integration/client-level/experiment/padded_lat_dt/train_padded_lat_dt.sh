...
# Train a standard DT model for padded_lat_dt pipeline.
# Output layout:
#   <trace_dir>/<dev0>...<dev1>/padded_lat_dt/training_results/
#   <trace_dir>/<dev0>...<dev1>/padded_lat_dt/training_results/padded_headers/
...
for TRACE_DIR in "$@"; do
    TRAINING_RESULTS_DIR="${TRACE_DIR}/${DEV0}...${DEV1}/padded_lat_dt/training_results"
    ...
    echo
    echo "======================================================="
    echo "Training padded-latency DT for workload:"
    ...
    # Prefer local padded_lat_dt datasets. If absent, backfill from flashnet.
    ...
    OUT_DIR="${TRAINING_RESULTS_DIR}/padded_headers"
    mkdir -p "${OUT_DIR}"
