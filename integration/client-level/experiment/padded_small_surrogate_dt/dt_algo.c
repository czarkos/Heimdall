...
// Small surrogate DT headers (depth-5, fidelity-trained).
#include "dt_weights_header/w_Trace_dev_0_dt.h"
#include "dt_weights_header/w_Trace_dev_1_dt.h"

// FlashNet headers used for shadow forward pass (result discarded).
...
int surrogate_dt_inference(long io_type, long size, uint32_t device, long cur_queue_len) {
    ...
    /* Shadow FlashNet forward pass to pad inference latency (result discarded). */
    ...
    if (VERBOSE) {
        printf("[padded_small_surrogate_dt][dev %u] hist=%ld pred=%d\n", device, cur_hist_index, pred);
    }
