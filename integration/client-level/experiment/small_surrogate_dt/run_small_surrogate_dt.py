...
def copy_small_surrogate_weights(
    trace_dir: str, devices_list: List[str], specific_workplace: str, algo_name: str
) -> bool:
    ...
    path_to_weights = os.path.join(
        dev_pair_dir, algo_name, "training_results", "surrogate_headers"
    )
...
def train_small_surrogate_dt(
    trace_dir: str, devices: List[str], max_depth: int, script_dir: str, algo_name: str
) -> bool:
    dev_names = [os.path.basename(d) for d in devices]
    train_script = os.path.join(script_dir, "train_small_surrogate_dt.sh")
    ...
    env = os.environ.copy()
    env["SMALL_SURROGATE_MAX_DEPTH"] = str(max_depth)
    env["SMALL_SURROGATE_ALGO_NAME"] = algo_name
    try:
        subprocess.run(train_command, check=True, env=env)
    except subprocess.CalledProcessError as train_e:
        print(f"Error running small surrogate DT training: {train_e}")
        return False
    return True
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=15,
        help="Max depth used for small surrogate DT training.",
    )
    parser.add_argument(
        "-algo_name",
        type=str,
    ...
    replay_source_dir = os.path.join(experiment_root, "surrogate_dt")
    ...
    for trace_dir in trace_dirs:
        ...
        dt_weights_dir = os.path.join(
            dev_pair_dir, args.algo_name, "training_results", "surrogate_headers"
        )
        ...
        if (
            args.resume
            and os.path.isfile(output_stat_path)
        ...
        if not args.only_replaying:
            if not (
                ...
                if not train_small_surrogate_dt(
                    trace_dir=trace_dir,
