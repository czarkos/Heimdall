...
def ensure_dt_shared_source(specific_workplace: str) -> bool:
    """
    padded_lat_dt/io_replayer.c and atomic.h include files from ../dt.
    Ensure that sibling directory exists in tmp workplace:
      <tmp_parent>/dt
    """
...
def resolve_dt_header_source_dir(trace_dir: str, devices_list: List[str]) -> str:
    ...
    Resolve DT header source directory with fallback:
      1) padded_lat_dt/training_results/padded_headers
    ...
    primary = os.path.join(
        dev_pair_dir, "padded_lat_dt", "training_results", "padded_headers"
    )
    ...
    if primary_ok:
        print(f"[padded_lat_dt] Using primary DT headers: {primary}")
        return primary
    ...
    if fallback_ok:
        print(f"[padded_lat_dt] Using fallback DT headers from dt: {fallback}")
        return fallback

    return ""


def copy_padded_lat_dt_headers(trace_dir: str, devices_list: List[str], specific_workplace: str) -> bool:
    dev_names = [os.path.basename(d) for d in devices_list]
    src_dir = resolve_dt_header_source_dir(trace_dir, devices_list)
    if src_dir == "":
        print(
            "DT headers missing in both padded_lat_dt and dt locations for trace: {}".format(
                trace_dir
            )
        )
        return False
    ...
    for dev_id in range(len(dev_names)):
        ...
        if not os.path.exists(src):
            print(f"padded_lat_dt header missing: {src}")
            return False
...
def train_padded_lat_dt(trace_dir: str, devices: List[str]) -> bool:
    dev_names = [os.path.basename(d) for d in devices]
    train_command = ["./train_padded_lat_dt.sh"] + dev_names
    ...
    print("padded_lat_dt training command: {}".format(train_command))

    original_directory = os.getcwd()
    try:
        os.chdir("./padded_lat_dt")
        subprocess.run(train_command, check=True)
    except subprocess.CalledProcessError as train_e:
        print(f"Error running padded_lat_dt training: {train_e}")
...
if __name__ == "__main__":
    ...
    for trace_dir in trace_dirs:
        ...
        flashnet_header_dir = os.path.join(dev_pair_dir, "flashnet", "training_results", "weights_header_2ssds")
        ...
        if args.resume and os.path.isfile(output_stat_path) and dt_headers_ready:
            print("Trace already trained and replayed for padded_lat_dt, skipping")
            continue

        time_start = pd.Timestamp.now()

        if not args.only_replaying:
            if not (args.resume and dt_headers_ready):
                if not train_padded_lat_dt(trace_dir, args.devices):
                    raise SystemExit(1)
                dt_headers_ready = (
                    os.path.isfile(output_dt_header_primary_0)
                    and os.path.isfile(output_dt_header_primary_1)
        ...
        if not dt_headers_ready:
            print("padded_lat_dt headers are not ready, skipping")
            continue
        ...
        if not ensure_dt_shared_source(specific_workplace):
            raise SystemExit(1)
        if not copy_padded_lat_dt_headers(trace_dir, args.devices, specific_workplace):
            raise SystemExit(1)
        ...
        try:
            ...
            try:
                ...
                print(f"Error running make in padded_lat_dt: {make_error}")
