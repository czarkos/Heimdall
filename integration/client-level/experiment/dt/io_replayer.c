...
// gcc dt_algo.c io_replayer.c -o io_replayer_dt -lpthread
// sudo ./io_replayer_dt /dev/nvme0n1 /home/.../trace /home/.../out.trace
enum {
    READ_IO = 1,
    WRITE_IO = 0,
};
...
/* Store each IO's latency, throughput and queue_len after each IO completes */
long *req_queue_len;
long *req_latency;
long *req_throughput;
/* Mark the finished status of each IO. Used for indicating whether the historical data of an IO is valid */
enum {
    IO_NOT_COMPLETED = -1,
    IO_REJECTED = -2,
    IO_HIST_APPENDED = -3,
...
void parse_io(char **reqs, int total_io) {
    ...
    if (oft == NULL || reqsize == NULL || reqflag == NULL ||
        timestamp == NULL || req_queue_len == NULL || req_latency == NULL || req_throughput == NULL) {
    ...
    for (i = 0; i < total_io; i++) {
        ...
        // init historical recording: req_queue_len, req_latency, req_throughput to be `IO_NOT_COMPLETED`
...
/*
    A client implementation of surrogate decision tree
*/
int surrogate_dt_algo(long io_type, long size, long cur_queue_len) {
    int target_device = original_device_index;

    int prediction_result = surrogate_dt_inference(io_type, size, target_device, cur_queue_len);   // input arguments: io_type, size, device
    if (io_type == READ_IO) {
        if (prediction_result == 1) { // if the prediction result is "1", means the IO is rejected, redirect it to another device.
    ...
    while (!shouldQuit()) {
        ...
        if (respecttime == 1) {
            ...
            if (elapsedtime < (int64_t)(timestamp[cur_idx] * 1000)) {
                ...
                // High slack rate is fine as long as late rate is 0%
                mylatecount++;
            }
        }
        // do the job
        ...
        // get the queue_len when submitting this IO.
        long cur_queue_len = add_fetch_cur_queue_len();
        int target_device = original_device_index;
        // If it is read IO, select its target device using surrogate decision tree:
        if (reqflag[cur_idx] == READ_IO){
            target_device = surrogate_dt_algo((long)reqflag[cur_idx], (long)reqsize[cur_idx], cur_queue_len);  // io_type, size, current_queue_length
        }

        /* If the io doesn't go to the original device, decrease the queue length by 1 */
        if (target_device != original_device_index) {
            dec_queue_len();
        }
        ...
        // Need to update the surrogate_dt, if the IO still goes to the original device
        // Both read and write IOs will update the historical data
        if (target_device == original_device_index){
            req_queue_len[cur_idx] = cur_queue_len;
            req_latency[cur_idx] = lat;
            req_throughput[cur_idx] = reqsize[cur_idx] / (lat + 1); // (lat + 1) to avoid divided by 0
            dec_queue_len(); // since the io is finished, decrease the queue length by 1.
        } else {
            // mark the historical data as IO_REJECTED
        ...
        /*
            Keep consistent with fio latency log format:
            1: timestamp in ms
            2: latency in us
            3: r/w type [0 for w, 1 for r]
            4: I/O size in bytes
            5: offset in bytes
        */
...
void *hist_update_thread() {
    /*
        Single thread to update the historical data, including:
            (1) historical queue length, (2) historical latency, (3) historical throughput.
    */
    long complete_idx = 0;   // all IOs with indices before `complete_idx` are completed.
    long cur_idx = 0;

    while (!shouldQuit()) {
        cur_idx = atomic_read(&jobtracker);
        /*
            Round-robin to check whether an IO finished.
            If the IO is finished, push it to the historical pool.
        */
        for (int i = complete_idx; i <= cur_idx; i++) {
            // check whether the historical data is valid
            if (req_queue_len[i] != IO_NOT_COMPLETED && req_latency[i] != IO_NOT_COMPLETED && req_throughput[i] != IO_NOT_COMPLETED) {
                ...
                // if the IO is both not rejected(-2) and not appended(-3), update the historical data for model inference.
                if (req_queue_len[i] >= 0 && req_latency[i] >= 0 && req_throughput[i] >= 0) {
                    // insert the latest value to historical pool. The historical pool is used by ml model to do inference.
                    update_surrogate_dt(req_queue_len[i], req_latency[i], req_throughput[i]);

                    // mark them as IO_HIST_APPENDED to avoid appending to historical pool again.
...
void do_replay(void) {
    ...
    pthread_t update_thread;  // single thread to update the historical data
    struct timeval t1, t2;
    float totaltime;
    int t;

    printf("Start doing IO replay with surrogate decision tree...\n");
...
int main(int argc, char **argv) {
    ...
    // set the surrogate decision tree model
